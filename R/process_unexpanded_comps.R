...
#' @export
#'
process_comps_unexpanded <- function(
  dir = NULL,
  datL,
  lgthBins = 1,
  ...
  if (lifecycle::is_present(sex)) {
    lifecycle::deprecate_warn(
      when = "2.2.1",
      what = "nwfscSurvey::UnexpandedLFs.fn(sex =)"
    )
  }

  plotdir <- file.path(dir, printfolder)
  #nwfscSurvey::check_dir(dir = plotdir, verbose = verbose)

  # Automatically find the needed columns
  sexn <- grep("sex", colnames(datL), ignore.case = TRUE, value = TRUE)[1]
  lenn <- grep("length", colnames(datL), ignore.case = TRUE, value = TRUE)[1]
  agen <- grep("age", colnames(datL), ignore.case = TRUE, value = TRUE)[1]
  yearn <- grep("year", colnames(datL), ignore.case = TRUE, value = TRUE)[1]
  ...
  if (length(lgthBins) == 1) {
    Lengths <- c(
      -999,
      seq(floor(min(datL[, lenn])), ceiling(max(datL[, lenn])), lgthBins),
      Inf
    )
  } else {
    ...
    if (length(Which) > 0) {
      ...
      for (s in c("F", "M")) {
        ...
        for (l in lgthBins) {
          ...
          if (l == min(lgthBins)) {
            Which2 <- Which[which(
              datL[Which, "allLs"] %in% c(l, -999) & datL[Which, sexn] == s
            )]
          }
          if (l != min(lgthBins)) {
            Which2 <- Which[which(
              datL[Which, "allLs"] == l & datL[Which, sexn] == s
            )]
          }
          if (l == max(lgthBins)) {
            Which2 <- Which[which(
              datL[Which, "allLs"] %in% c(l, Inf) & datL[Which, sexn] == s
            )]
          }
  ...
  if (length(datL[datL[, sexn] == "U", sexn]) > 0) {
    ...
    colnames(out_u)[-c(1:6)] <- paste(
      rep("U", each = length(lgthBins)),
      lgthBins,
      sep = ""
    )
  }
