...
#    dplyr::filter(n > 300) |> remove this filter for squarespot? squarespot has 218
    dplyr::ungroup()
  ...
  test <- species_to_plot[which(species_to_plot %in% c("squarespot rockfish"))]
  for (sp in test) {
...
#test <- species_to_plot[which(species_to_plot %in% c("big skate"))]
