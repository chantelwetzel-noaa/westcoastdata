plot_additional_wcgbt_comps <- function(
    dir = here::here("plots", "additional_wcgbts_comps"),
    wcgbt_catch,
    wcgbt_bio,
    verbose = TRUE
) {
...
#    dplyr::filter(n > 300) |> remove this filter for squarespot
    dplyr::ungroup()
  ...
  test <- species_to_plot[which(species_to_plot %in% c("big skate", "squarespot rockfish"))]
  for (sp in test) {
    catch <- wcgbt_catch[wcgbt_catch$Common_name == "big skate", ]
    if (length(unique(catch[, "Trawl_id"])) != dim(catch)[1]) {
      catch <- nwfscSurvey::combine_tows(data = catch)
    }
    bio <- wcgbt_bio[wcgbt_bio$Common_name == "big skate", ]
